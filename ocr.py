...
def ocr_raw(image):
    ...
    th, threshed = cv2.threshold(blackhat, 127, 255, cv2.THRESH_TRUNC)
...
def main(image):
    ...
    for tmp_data in last_result_list:
        ...
        if 'Status' in tmp_data or 'Perkawinan' in tmp_data:
            ...
            #         status_perkawinan = tmp_data[tmp_index + 1]
            #         if status_perkawinan == ":":
            #             status_perkawinan = tmp_data[tmp_index + 2]
            try:
        ...
        if 'Kel/Desa' in tmp_data:
            for tmp_index in range(len(tmp_data)):
                ...
                kel_desa = re.sub('[^A-Z0-9. ]', '', kel_desa)
        ...
        if 'Kecamatan' in tmp_data:
            for tmp_index in range(len(tmp_data)):
                ...
                kecamatan = re.sub('[^A-Z0-9. ]', '', kecamatan)
        ...
        if 'Tempat' in tmp_data or 'Tgl' in tmp_data or 'Lahir' in tmp_data:
            ...
            try:
                ...
                tempat_lahir = re.findall('\s+([A-Z]+)', tempat_lahir)
                tempat_lahir = ' '.join(tempat_lahir)
            except:
                tempat_lahir = ""
